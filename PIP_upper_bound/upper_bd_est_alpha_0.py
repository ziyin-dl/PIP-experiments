...
try:
  ...
  for run_idx in range(num_runs):
    ...
    for keep_dim in range(1, rank):
      ...
      bias = rank - keep_dim
      print((np.outer(D[:keep_dim], np.ones((n - rank, 1)))).shape)
      print(np.outer(np.ones((keep_dim, 1)), D[rank:]).shape)
      spectrum_T_inv = 1 / (np.outer(D[:keep_dim], np.ones((n - rank, 1))) - 
        np.outer(np.ones((keep_dim, 1)), D[rank:]))
      ...
      var_subspace_expansion = np.sqrt(2) * sigma * spectrum_T_inv_norms[-1]
      var_subspace_expansion_gt = np.sqrt(2) * spectrum_norms_gt[-1]
      var_subspace_ub.append(var_subspace_expansion)
      var_subspace_gt.append(var_subspace_expansion_gt)
      
      telescoping_expansion.append(np.sqrt(bias + var_subspace_expansion**2))
      gt_ub.append(np.sqrt(bias + var_subspace_expansion_gt**2))
...
for i in range(len(telescoping_expansion_dict)):
  ...
  if i == 0:
    ...
    # ax.plot(gt_ub, 'green', label = r'PIP Error Bound by Main Theorem')
    ax.plot(frobenius_list_est_to_gt, 'aqua', label = r'PIP Error by Theorem 2')
    ax.plot(telescoping_expansion, 'red', label = r'Actual PIP Error')
  else:
    # ax.plot(telescoping_ub, 'yellow')
    # ax.plot(gt_ub, 'green')
    ax.plot(frobenius_list_est_to_gt, 'aqua')
    ax.plot(telescoping_expansionm, 'red')
