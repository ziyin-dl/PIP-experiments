...
if __name__ == "__main__":
    ...
    parser.add_argument('--conf_ints', nargs = '+', default = 0.2, type = float)
    
    args = parser.parse_args()
    conf_ints = args.conf_ints
    ...
    with open('sv_{}.pkl'.format(spec_data), 'r') as f:
        D = pickle.load(f)
    n = len(D)
    shape = (n, n)
    ...
    dir_name = '{}/alpha_{}_sigma_{}_rank_{}'.format(spec_data, alpha, sigma, rank)
    ...
    '''last plot: overall bound quality vs ground truth'''
    fig = plt.figure()
    ax  = fig.add_subplot(111)
    level_sets = []
    ...
    for level, pct, endpoints, color in zip(level_sets, conf_ints, level_sets_endpoints, colors):
      ...
      plt.axhline(y=level, xmin=(endpoints[0] - xlim[0]) / (xlim[1] - xlim[0]), 
        ...
        label='+{}'.format(pct * 100) + r'percent', ls='--', c=color)
    lgd = ax.legend(loc='upper right')
    plt.xlabel('Dimensions')
    plt.ylabel('PIP Error')
    plt.title(r'PIP Error Ground Truth'.format(alpha))
