...
for k in args.k:
  ...
  SPPMI_test = np.maximum(PMI_test - np.log(k), 0.0)
  diff = SPPMI - SPPMI_test
  print("k={}: mean is {}".format(k, np.mean(diff)))
  print("k={}: est std is {}".format(k, 0.5 * np.std(diff)))
