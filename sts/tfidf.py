...
def read_data(filename):
  ...
  with open(filename, 'r') as f:
    for idx, line in enumerate(f):
      ...
      num_tokens1 += len(data1[-1])
      num_tokens2 += len(data2[-1])
      # print(data1[-1])
      # print(data2[-1])
      vocabulary += data1[-1]
      vocabulary += data2[-1]
      score.append(float(row[3]))
  print('first half has {} tokens'.format(num_tokens1))
  print('second half has {} tokens'.format(num_tokens2))
